...
export const Chat: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-100px)]">
      {/* Header */}
      <Card className="bg-brand-800 p-4 rounded-t-xl border-b border-brand-700 flex justify-between items-center">
        <div>
        ...
        <Button
          ...
          size="sm"
          className="bg-brand-500 hover:bg-brand-400 text-xs"
        >
          Item Giving
        </Button>
      </Card>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-brand-900/50">
        ...
        {messages.map(msg => (
          <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] p-3 rounded-xl text-sm ${msg.sender === 'me' ? 'bg-brand-600 text-white rounded-tr-none' : 'bg-brand-800 text-slate-200 rounded-tl-none'
              }`}>
              {msg.text}
            </div>
          </div>
      ...
      <form onSubmit={handleSend} className="p-3 bg-brand-800 border-t border-brand-700 flex space-x-2">
        ...
        />
        <Button type="submit" variant="secondary" className="bg-brand-600">Send</Button>
      </form>
